...
| 方法 | 优点 | 缺点 | 使用场景 |
| --- | --- | --- | --- |
| `linear` | 绝不过冲、结果最稳；改动一个观测点只影响相邻段 | 节点处有折角、不光滑，精度最低 | 快速估算、分段变化的量、作为其他方法的对照基线 |
| `cubic` | 二阶连续、曲线光滑，精度最高 | 陡变附近可能过冲出观测范围；非局部——改动一个点整条曲线都变 | 平滑物理量的取值与绘图（温度、轨迹、地形线） |
| `pchip` | 保单调、保局部形状，不会过冲 | 只有一阶连续，光滑度低于 `cubic` | 单调工程数据；不允许出现物理不合理值（如负浓度）时 |
