...
| 函数 | 优点 | 缺点 | 使用场景 |
| --- | --- | --- | --- |
| `missing_report` | 零成本暴露数据质量全貌，为「删列还是填充」提供决策依据 | 只统计数量，看不出缺失机制（是否随机缺失） | 拿到数据后的第一步检查，论文预处理小节的第一张表 |
| `drop_duplicates` | 消除重复样本导致的评估虚高与统计量偏差 | 识别不了「关键字段相同、其余字段略有出入」的近重复 | 建模与数据划分前的例行步骤，尤其是多源合并、爬取得来的数据 |
| `handle_missing` | 中位数对偏态和异常值稳健；保留全部样本量，不删行 | 会压低方差、弱化变量间相关；只处理数值列，缺失比例高的列填充后信息量有限 | 数值列缺失比例不高（经验上 <30%）、缺失近似随机时的默认填充 |
...
| 方法 | 优点 | 缺点 | 使用场景 |
| --- | --- | --- | --- |
| IQR 法 | 不依赖分布假设；边界由四分位数决定，极端值拉不动它 | 不考虑数值出现的概率，多峰分布下可能把正常模式判为异常 | 默认首选；收入、面积等偏态指标的异常筛查 |
| 3σ 法 | 与正态理论衔接直观，近似正态数据下边界合理 | 偏态 / 重尾数据下掩蔽效应明显，漏检多于 IQR | 近似正态的列：测量误差、质量控制数据 |
...
| 编码 | 优点 | 缺点 | 使用场景 |
| --- | --- | --- | --- |
| one-hot | 不引入虚假的顺序与距离关系，任何模型都能用 | 类别多时维度爆炸、矩阵稀疏，并加剧线性模型的共线性 | 无序类别（颜色、城市、型号）；类别数不多时通用 |
| 标签编码 | 不增加列数，保持单列存储，信息紧凑 | 给无序类别引入虚假的大小关系，会误导线性 / 距离类模型 | 有序类别（低/中/高）；树模型（随机森林、XGBoost）的特征输入 |
