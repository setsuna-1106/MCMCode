...
- **优点**：BFS / DFS 都是 $O(V+E)$ 的线性时间算法，实现简单；连通分量直接回答「网络分成几块」。
- **缺点**：不考虑边权，BFS 的「最短」只是跳数最少；递归 DFS 在大图上可能栈溢出。
- **使用场景**：网络结构描述与可视化、可达性判断、把大网络拆成连通子问题再分别处理。
...
- **优点**：非负权下保证最优，复杂度稳定可预期；`weight=None` 即退化为跳数最少，一个接口两用。
- **缺点**：负权边下结果不正确（须换 Bellman-Ford）；每次查询独立求解，多对起终点时重复计算。
- **使用场景**：运输、导航、管网、通信路由等求成本 / 距离最小路径的问题。
...
- **优点**：贪心算法有最优性保证；只用 $n-1$ 条边实现最低成本连通，结果可直接解释为建设方案。
- **缺点**：只适用于无向图；只优化「连通成本」，不保证可靠性（单边故障即断开）也不考虑流量需求。
- **使用场景**：铺路 / 布线 / 管网设计的最低成本方案；作为网络骨架分析与单链接聚类的底层工具。
...
- **优点**：多项式时间求最优；最大流 = 最小割，一次求解同时得到「最多能送多少」和「瓶颈在哪」。
- **缺点**：单源单汇（多源多汇要加超级源 / 汇点）；只处理单一种类流；容量变化后需整体重算。
- **使用场景**：物流运力上限、管网 / 网络吞吐瓶颈分析、疏散与撤离能力评估。
...
- **优点**：严格一对一的最优配对；权重语义灵活（收益、成本、契合度都可）；支持一般图（不限于二分图）。
- **缺点**：只能表达一对一关系，容量、多对多等复杂约束表达不了（需转[优化模块](optimization.md)的 CP-SAT / 最小费用流）。
- **使用场景**：人员-任务指派、导师-学生结对、机组-航线分配等一对一配对问题。
...
- **优点**：一次求出总工期与关键链，直接回答「哪些工序不能延误、哪些有余量」；算法高效。
- **缺点**：要求 DAG 且工序工期确定，不处理工期不确定性（需要 PERT 或仿真）；只优化总工期单一目标。
- **使用场景**：项目排期与工序依赖分析——总工期预测、找瓶颈工序、评估「赶工哪道工序最有效」。
...
| 指标 | 优点 | 缺点 | 使用场景 |
| --- | --- | --- | --- |
| 度中心性 | $O(V)$ 计算最便宜，直觉最好 | 只看直接邻居，衡量不到「桥梁」作用 | 快速找局部枢纽 |
| 介数中心性 | 捕捉「桥梁 / 咽喉」节点 | $O(VE)$，大图计算慢 | 找交通要道、关键中转站 |
| 接近中心性 | 反映到达全网的平均便利度 | 不连通图上无定义（需按分量处理） | 选址类问题：急救站、仓库 |
| PageRank | 考虑指向者的质量，适合有向网络 | 阻尼系数与收敛阈值需设定，对悬挂节点敏感 | 引文网络、链接排名、影响力识别 |
