...
- **优点**：自适应细分对平滑函数极高效，通常几十次求值达到 $10^{-9}$ 精度；自带误差估计，结果可信度可直接检查；原生支持反常积分（±∞、端点奇异）。
- **缺点**：强震荡或奇异函数可能收敛慢甚至失败，需人工用 `points` 分段；只处理一维，高维要换 `dblquad` 等接口。
- **使用场景**：模型中的定积分计算——概率与期望、面积 / 体积、变力做功，以及微分方程解的积分量。
