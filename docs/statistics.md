...
- **优点**：不要求两组方差相等，比经典 Student t 检验适用面更广；输出直观（t 值、自由度、p 值），是两组均值比较的默认选择。
- **缺点**：仍要求每组数据近似正态，样本量太小时近似变差；一次只能比较两组。
- **使用场景**：实验组 vs 对照组、两个地区 / 两种工艺的指标对比等两组独立样本的均值比较。
...
- **优点**：配对设计剔除了对象间固有差异，同样本量下比独立样本检验更容易检出真实差异。
- **缺点**：只适用于同一对象前后测量（或严格配对）的设计；差值仍需近似正态；缺一个配对对象就损失一对数据。
- **使用场景**：处理前 / 后指标、同一样品两种方法测量、同一对象左右对照。
...
- **优点**：一次检验即可比较多组方差；`center="median"` 版本对非正态数据稳健。
- **缺点**：只回答「方差是否齐」这一个问题，不比较均值；结果仍可能被个别极端离群值拉动。
- **使用场景**：经典 ANOVA、pooled t 检验前的方差齐性前置检查（Welch t 检验不需要）。
...
- **优点**：适用于任意规模的列联表；样本量充足时结果可靠，是分类变量关联分析的标准工具。
- **缺点**：期望频数过小时结果不可靠（见「注意」）；只回答「是否关联」，不给出关联强度与方向。
- **使用场景**：问卷交叉表分析（性别 × 偏好、地区 × 购买与否）等两个分类变量的关联检验。
...
- **优点**：精确检验不依赖大样本渐近，小样本与低频数场景下比卡方可靠；附带优势比可报告效应大小。
- **缺点**：只支持 2x2 表；大样本下与卡方结论一致，没有额外收益。
- **使用场景**：小样本试验、罕见事件（阳性数很少）的 2x2 表比较。
...
- **优点**：用一次检验比较多组均值，避免逐对 t 检验造成的假阳性累积；方差分解思想清晰，可延伸到双因素、协方差分析。
- **缺点**：要求各组近似正态且方差齐；显著时只知道「至少两组不同」，还需事后检验定位差异。
- **使用场景**：三种及以上处理 / 方案 / 地区的指标比较，是「多组谁更好」问题的第一步。
...
- **优点**：族错误率受控，一次给出全部组对的结论；均值差置信区间可直接写进论文。
- **缺点**：假设各组方差齐、样本量接近；多重校正后单个比较的功效低于逐对 t 检验。
- **使用场景**：ANOVA 拒绝原假设后，回答「具体哪几组之间存在差异」。
...
| 检验 | 优点 | 缺点 | 使用场景 |
| --- | --- | --- | --- |
| `shapiro_wilk` | 小样本下功效最高，对偏离正态最灵敏 | 仅适用 $3 \le n \le 5000$；大样本下过于灵敏 | 默认首选，尤其 n < 50 |
| `jarque_bera_test` | 只依赖偏度与峰度，大样本渐近有效，附带两个描述统计量 | 小样本下功效低 | 大样本（超出 SW 适用范围时） |
| `lilliefors_test` | 参数由样本估计时 K–S 的正确版本 | 功效通常低于前两者 | 需要 K–S 框架或与文献口径对齐时 |
...
- **优点**：不要求正态、对离群值稳健、适用于只能排序打分的序数数据，是分布形态未知时的安全选择。
- **缺点**：数值转秩损失了距离信息，正态假设成立时功效低于对应的参数检验；结论只能说「分布位置不同」。
- **使用场景**：正态性检验不通过、样本量小、问卷等级评分、明显偏态的指标（收入、时长）。
...
| 方法 | 优点 | 缺点 | 使用场景 |
| --- | --- | --- | --- |
| `pearson_correlation` | 度量线性关系的强度与方向，计算简单、附显著性检验 | 对异常值敏感；弯曲关系会低估关联 | 关系近似直线、数据质量好时 |
| `spearman_correlation` | 秩变换后稳健，能捕捉单调关系，不受单调变换与离群值影响 | 忽略数值间距，无法刻画非单调（如 U 形）关系 | 指数型增长、含离群值或序数变量 |
...
- **优点**：不假设任何分布形状，任意统计量（中位数、分位数、自定综合指标）都能给出区间；思路通用，代码量小。
- **缺点**：计算量比解析公式大；极小样本下经验分布代表性不足，区间覆盖率可能不够。
- **使用场景**：论文中要报告「中位数及其置信区间」这类没有解析标准误的统计量，或对分布假设没有把握时。
